...
calc_growth_rate <- function(country_vector, type_cases = "Total_cases", cases_vs_tests = FALSE){
  sym_cases <- rlang::sym(type_cases)
  
  if(cases_vs_tests){
    growth_rate <- sapply(country_vector, function(x) 
      data %>% filter(Country_code == x) %>%
        mutate(cases_per_day = !!sym_cases - lag(!!sym_cases)) %>% 
        mutate(tests_per_day = Tests - lag(Tests)) %>%
        #mutate(tests_per_day = if_else(tests_per_day == 0, 1, tests_per_day)) %>%
        mutate(rate_diff = cases_per_day/tests_per_day) %>% 
        pull(rate_diff)) %>% as.data.frame(stringsAsFactor = FALSE)
    
    growth_rate <- mutate(growth_rate, Date = unique(data$Date))
  }
  else{
  growth_rate <- sapply(country_vector, function(x) 
    data %>% filter(Country_code == x ) %>% group_by(Date, Country_code) %>% 
      summarise(cases = sum(!!sym_cases) + 1, .groups = "drop") %>% 
      mutate(day_diff = as.integer(Date - lag(Date)), growth_diff = cases - lag(cases), 
             rate_diff = (growth_diff/day_diff)/lag(cases) * 100) %>% 
      pull(rate_diff)) %>% as.data.frame(stringsAsFactor = FALSE)
  ...
  growth_rate <- growth_rate %>% gather(key = "Country_code", value = GrowthRate, -Date) %>%
    mutate(GrowthRate = round(GrowthRate,2)) 
  return(growth_rate)
}
...
doublingRate <- function(df){
  ...
  #doubleTimeVector <- map_dbl(index_gt0:index_toplimit, function(i) 
  #detect_index(df[,"Total_cases"], function(y) y >= df[i,"twice"]) - i)
  # =============Slow code end =================
    doubleTimeVector <- map_dbl(index_gt0:index_toplimit, function(i) 
    which.max(df[,"Total_cases"] >= df[i,"twice"]) - i)
...
# Load the data
cases <- read_csv("data/cases/time_series_covid19_confirmed_global_iso3_regions15-10-2020.csv", show_col_types = FALSE)
# Rearrange the columns for cases
cases <- rearrange_col(cases)
# Deaths data
deaths <- read_csv("data/deaths/time_series_covid19_deaths_global_iso3_regions15-10-2020.csv", show_col_types = FALSE)
deaths <- rearrange_col(deaths)
# Recovered data
recovered <- read_csv("data/recovered/time_series_covid19_recovered_global_iso3_regions15-10-2020.csv", show_col_types = FALSE)
recovered <- rearrange_col(recovered)
# tests
test <- read_csv("data/tests/covid-19-total-confirmed-cases-vs-total-tests-conducted-15-10-2020.csv", 
         col_names = TRUE, col_types = cols("Entity" = col_character(), "Code" = col_character(), 
                                            #Date = col_character(),
                                            "Date" = col_date(format = "%Y-%m-%d"), 
                                            `Total tests (tests)` = col_double(), 
...
test <- test[!is.na(test$Date),]
test$Date <- format.Date(test$Date, "%m-%d-%Y")
...
global_data <- left_join(global_data, df_recovered %>% 
            group_by(Date) %>% 
            summarize(recovered = sum(Recovered), .groups = "drop_last"), by = "Date")

# add the test data to the global data
global_data <- left_join(global_data, test %>% 
                           group_by(Date) %>% 
                           summarise(test = sum(Tests), .groups = "drop_last"), by = "Date")
...
data <- df %>% group_by(Date,Country_code) %>% 
  summarise(Total_cases = sum(Cases), Deaths = sum(Deaths), .groups = "drop") %>%
...
data <- left_join(data, df_recovered %>% 
            ...
            summarise(Recovered = sum(Recovered), .groups = "drop"), by = c("Date","Country_code"))

# add test country wise data 
data <- left_join(data, test %>%
            group_by(Date, Country_code) %>%
            summarise(Tests = sum(Tests), .groups = "drop"), by = c("Date","Country_code"))
...
# Let us calculate the growth_rate 
unique_country_codes <- unique(data$Country_code)

growth_rateT <- calc_growth_rate(unique_country_codes,"Total_cases") %>% rename(GrowthRate_Total_cases = "GrowthRate")
growth_rateA <- calc_growth_rate(unique_country_codes,"Active_cases") %>% rename(GrowthRate_Active_cases = "GrowthRate")

growth_rate <- inner_join(growth_rateT, growth_rateA, by = c("Date","Country_code"))
growth_rate <- inner_join(growth_rate, character_col, by = c("Country_code"))

rm(growth_rateT, growth_rateA)
...
ui <- navbarPage("Covid 19",selected = "Cases",inverse = TRUE, collapsible = TRUE,
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                selectizeInput(inputId = "country1", label = "Choose country:", 
                                               ...
                                               selected = c("United States (USA)", "Brazil (BRA)","India (IND)"), 
                 ...
                 tabPanel("Growth", 
                          fluidPage(
                            sidebarLayout(
                              sidebarPanel(
                                radioButtons(inputId = "radioGrowth", label = "Choose growth in", 
                                             choices = c("Total Cases", "Active Cases"), 
                                             selected = "Total Cases", inline = TRUE),
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                selectizeInput(inputId = "country3", label = "Choose country:", 
                                               choices = c(sort(unique(data$Country_code))), 
                                               selected = c("Brazil (BRA)","United States (USA)","India (IND)"), 
                                               multiple = TRUE, options = list(placeholder = "Please select countries below")),
                 ...
                 tabPanel("Cases vs Tests",
                          fluidPage(
                            sidebarLayout(
                              sidebarPanel(
                                # Cases vs Tests radio
                                # Radio input
                                radioButtons("radioCasesTests", label = "Choose confirmed cases", 
                                             choices = list("Total" = "total", "Active" = "active"), selected = "total", inline = TRUE),
                                # Select Country
                                selectizeInput(inputId = "country6", label = "Choose country:",
                                               choices = c(sort(unique(data$Country_code))),
                                               selected = c("India (IND)","United States (USA)","World (WOR)"),
                                               multiple = TRUE, options = list(placeholder = "Please select countries below")),
...
server <- function(input, output) {
  ...
  output$lineplot2 <- renderPlotly({
    ...
    if (input$radioGrowth == "Total Cases")
    {
      subdata1 <- subdata1 %>% group_by(Country_code) %>%
        mutate(rollingMean = rollapply(GrowthRate_Total_cases, 5, mean, by = 1, align = "right", fill = NA)) %>% 
        mutate(rollingMean = round(rollingMean,2)) %>% rename(Rate_of_Growth = GrowthRate_Total_cases)
    }
    else{
      subdata1 <- subdata1 %>% group_by(Country_code) %>%
        mutate(rollingMean = rollapply(GrowthRate_Active_cases, 5, mean, by = 1, align = "right", fill = NA)) %>% 
        mutate(rollingMean = round(rollingMean,2)) %>% rename(Rate_of_Growth = GrowthRate_Active_cases)
    }
  ...
  output$table2 <- renderDataTable({
    ...
    if (is.element("Total Cases", input$radioGrowth))
    {
      subdata2 <- subdata2  %>% group_by(Country_code) %>%
        mutate(rollingMean = rollapply(GrowthRate_Total_cases, 5, mean, by = 1, align = "right", fill = NA)) %>% 
        mutate(rollingMean = round(rollingMean,2)) %>% rename(Rate_of_Growth = GrowthRate_Total_cases) %>% ungroup()
    }
    else {
      subdata2 <- subdata2  %>% group_by(Country_code) %>%
        mutate(rollingMean = rollapply(GrowthRate_Active_cases, 5, mean, by = 1, align = "right", fill = NA)) %>% 
        mutate(rollingMean = round(rollingMean,2)) %>% rename(Rate_of_Growth = GrowthRate_Active_cases) %>% ungroup()
    }
  ...
  output$lineplot6 <- renderPlotly({
    ...
    if (input$radioCasesTests == "total")
    {
      subdata6 <- calc_growth_rate(unique(subdata6$Country_code), type_cases = "Total_cases", cases_vs_tests = TRUE) %>%
        rename(Cases_vs_Tests = GrowthRate)
    }
    else {
      subdata6 <- calc_growth_rate(unique(subdata6$Country_code), type_cases = "Active_cases", cases_vs_tests = TRUE) %>%
        rename(Cases_vs_Tests = GrowthRate)
    }
    
    subdata6 <- subdata6 %>% filter(Cases_vs_Tests > -1, Cases_vs_Tests < 1) %>% 
      filter(Date >= lower_date & Date <= upper_date)
